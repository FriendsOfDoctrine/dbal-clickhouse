...
class ClickHousePlatform extends AbstractPlatform
{
    ...
    public function getAlterTableSQL(TableDiff $diff): array
    {
        ...
        if ($diff->newName !== false || !empty($diff->renamedColumns)) {
            throw ConnectionException::notSupported('RENAME COLUMN');
        }
    ...
    public function getDefaultValueDeclarationSQL($field): string
    {
        ...
        if ($fieldType !== null) {
            if (
                ...
                $fieldType === DatableClickHouseType::TYPE_DATE_TIME &&
    ...
    protected function _getCreateTableSQL($tableName, array $columns, array $options = []): array
    {
        ...
        if (isset($options['uniqueConstraints']) && !empty($options['uniqueConstraints'])) {
            throw ConnectionException::notSupported('uniqueConstraints');
        }
        ...
        if (
            ...
            if (empty($options['eventDateColumn'])) {
                $dateColumns = array_filter($columns, fn($column) => $column['type'] instanceof DateType);
